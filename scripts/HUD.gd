...
func _ready() -> void:
    ...
    subtitle.text = "MISSÃO 01 · MÓDULO DE PREPARAÇÃO · T−00:17:42"
    subtitle.position = Vector2(34, 57)
    subtitle.add_theme_font_size_override("font_size", 12)
    subtitle.add_theme_color_override("font_color", Color("66889a"))
    ...
    for key in ["energy", "oxygen", "fuel", "temperature", "radiation"]:
        var label := Label.new()
